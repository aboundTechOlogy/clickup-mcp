...
export class OAuthStorage {
  ...
  constructor(dbPath?: string) {
    const storagePath = dbPath || '/opt/ai-agent-platform/mcp-servers/clickup-mcp/oauth.db';
    this.db = new Database(storagePath);
    this.initSchema();
    console.log('OAuth storage initialized:', storagePath);
  }
  ...
  private cleanupExpired(): void {
    ...
    if (deletedCodes.changes > 0 || deletedTokens.changes > 0) {
      console.log('Cleaned up expired OAuth data:', {
